...
describe('ListConversationsController', () => {
  ...
  beforeEach(() => {
    listConversationsUseCase = {
      execute: vi.fn(),
    } as unknown as ListConversationsUseCase;
  ...
  it('should call use case and return result', async () => {
    ...
    expect(listConversationsUseCase.execute).toHaveBeenCalledWith({
      userId: 'u-1',
    });
  ...
  it('should return empty conversations array', async () => {
    vi.mocked(listConversationsUseCase.execute).mockResolvedValue({
      conversations: [],
    });
