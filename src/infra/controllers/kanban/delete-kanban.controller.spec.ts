...
describe('DeleteKanbanController', () => {
  ...
  beforeEach(() => {
    deleteKanbanUseCase = {
      execute: vi.fn(),
    } as unknown as DeleteKanbanUseCase;
  ...
  it('should call use case and return status ok', async () => {
    ...
    expect(deleteKanbanUseCase.execute).toHaveBeenCalledWith({
      kanbanId: 'k-1',
      userId: 'u-1',
    });
  ...
  it('should propagate errors from the use case', async () => {
    vi.mocked(deleteKanbanUseCase.execute).mockRejectedValue(
      new Error('Kanban not found'),
    );
    await expect(
      controller.handle('k-1', { user: { id: 'u-1' } } as any),
