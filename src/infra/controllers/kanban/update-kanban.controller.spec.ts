...
describe('UpdateKanbanController', () => {
  ...
  beforeEach(() => {
    updateKanbanUseCase = {
      execute: vi.fn(),
    } as unknown as UpdateKanbanUseCase;
  ...
  it('should call use case and return status ok', async () => {
    ...
    const result = await controller.updateKanban(
      { id: 'k-1', title: 'T', description: 'D', imageUrl: null },
      req,
    );

    expect(updateKanbanUseCase.execute).toHaveBeenCalledWith({
      id: 'k-1',
      userId: 'u-1',
      title: 'T',
      description: 'D',
  ...
  it('should propagate errors from the use case', async () => {
    vi.mocked(updateKanbanUseCase.execute).mockRejectedValue(
