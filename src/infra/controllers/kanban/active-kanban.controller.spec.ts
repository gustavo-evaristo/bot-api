...
describe('ActiveKanbanController', () => {
  ...
  beforeEach(() => {
    activeKanbanUseCase = { execute: vi.fn() } as unknown as ActiveKanbanUseCase;
  ...
  it('should call use case and return status ok', async () => {
    ...
    expect(activeKanbanUseCase.execute).toHaveBeenCalledWith({ id: 'k-1', userId: 'u-1' });
    expect(result).toEqual({ status: 'ok' });
  });

  it('should propagate errors from the use case', async () => {
    vi.mocked(activeKanbanUseCase.execute).mockRejectedValue(new Error('Phone number is required to activate the kanban'));
    await expect(controller.handle({ id: 'k-1' }, { user: { id: 'u-1' } } as any)).rejects.toThrow('Phone number is required');
  });
});
