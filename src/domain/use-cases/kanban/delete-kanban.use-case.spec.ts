...
describe('DeleteKanbanUseCase', () => {
  ...
  it('should throw if user not found', async () => {
    ...
    await expect(useCase.execute({ kanbanId: 'k-1', userId: 'u-1' })).rejects.toThrow('User not found');
  });
  ...
  it('should throw if kanban not found', async () => {
    ...
    await expect(useCase.execute({ kanbanId: 'k-1', userId: 'u-1' })).rejects.toThrow('Kanban not found');
  });
  ...
  it('should throw if user is not the owner', async () => {
    ...
    await expect(useCase.execute({ kanbanId: kanban.id.toString(), userId: user.id.toString() })).rejects.toThrow('User is not the owner of the kanban');
  });
  ...
  it('should soft-delete the kanban', async () => {
    ...
    await useCase.execute({ kanbanId: kanban.id.toString(), userId: user.id.toString() });
